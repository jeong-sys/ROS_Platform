...
function maybeStart() {
  ...
  if (!isStarted && typeof localStream !== 'undefined' && isChannelReady) {
    ...
    if (isInitiator) {
      dataChannel = pc.createDataChannel('chat'); // Initiator인 경우 DataChannel 생성
      setupDataChannel(); // 채널 설정
      doCall();
    }
  }
}
...
function createPeerConnection() {
  try {
    ...
    pc.ondatachannel = receiveChannelCallback; // 비시작자는 이 콜백으로 데이터 채널 수신
...
function setupDataChannel() {
  ...
  dataChannel.onmessage = function(event) {
    ...
    displayChatMessage(event.data); // 채팅 메시지 화면에 표시
  };
}
